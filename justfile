...
# Backup saves to local project folder with a timestamp.
backup:
    mkdir -p backups
    docker compose exec -T dwarf-fortress sh -c "tar -czf /opt/df/backups/save-\$(date +%Y%m%d_%H%M%S).tar.gz -C /opt/df/data save"
...
# Restore a backup from the backups folder. Usage: just restore FILE="save-20260111_120000.tar.gz"
restore FILE WAIT="10":
    @echo "Targeting backup: backups/{{FILE}}"
    @echo "WARNING: This will overwrite your current save! (Ctrl+C to abort)"
    @echo "{{WAIT}} seconds to abort..."
    @sleep {{WAIT}}
    @echo "Restoring"
    @echo "Stopping server..."
    docker compose stop dwarf-fortress
    @echo "Restoring..."
    docker compose run -T --rm dwarf-fortress sh -c "rm -rf /opt/df/data/save/* && tar -xzf /opt/df/backups/{{FILE}} -C /opt/df/data"
...
smoke-test:
    ...
    docker compose exec -T -e DISPLAY=:1 dwarf-fortress xdotool search --name "Dwarf Fortress" windowactivate --sync
    
    @echo "Clicking 'Create New World' button..."
    docker compose exec -T -e DISPLAY=:1 dwarf-fortress xdotool mousemove 640 440 mousedown 1 sleep 0.2 mouseup 1
    @sleep 3
    
    @echo "Clicking 'Okay' on the welcome prompt..."
    docker compose exec -T -e DISPLAY=:1 dwarf-fortress xdotool mousemove 640 300 mousedown 1 sleep 0.2 mouseup 1
    @sleep 3
    
    @echo "Selecting 'Pocket' world size..."
    docker compose exec -T -e DISPLAY=:1 dwarf-fortress xdotool mousemove 100 80 mousedown 1 sleep 0.2 mouseup 1
    @sleep 1
    
    @echo "Confirming world generation..."
    docker compose exec -T -e DISPLAY=:1 dwarf-fortress xdotool mousemove 800 750 mousedown 1 sleep 0.2 mouseup 1
    @sleep 1
    ...
    docker compose exec -T -e DISPLAY=:1 dwarf-fortress xdotool mousemove 300 275 mousedown 1 sleep 0.2 mouseup 1
    @sleep 3